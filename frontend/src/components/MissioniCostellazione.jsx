...
// Posizioni predefinite costellazione (unità scena)
...
function makeGalaxySpiral(count, arms, spread, radius) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    pos[i * 3 + 1] = (Math.random() - 0.5) * scatter * 0.4;
...
export const MissioniCostellazione = ({ missionsData, onBack }) => {
  ...
  useEffect(() => {
    ...
    const palette = [
      ...
      new THREE.Color(0xffeebb), new THREE.Color(0xccaaff),
    ];
    for (let i = 0; i < N; i++) {
    ...
    const bgStars = new THREE.Points(bgGeo, new THREE.PointsMaterial({
      size: isMob ? 0.07 : 0.055, vertexColors: true,
      transparent: true, opacity: 0.75, sizeAttenuation: true,
    }));
    scene.add(bgStars);

    // ── Galassie a spirale ────────────────────────────────────────────────────
    const addGalaxy = (count, arms, spread, radius, posArr, rotZ, opacity) => {
      const { pos, col } = makeGalaxySpiral(count, arms, spread, radius);
      const geo = new THREE.BufferGeometry();
      geo.setAttribute("position", new THREE.BufferAttribute(pos, 3));
      geo.setAttribute("color",    new THREE.BufferAttribute(col, 3));
      const pts = new THREE.Points(geo, new THREE.PointsMaterial({
        size: 0.06, vertexColors: true, transparent: true, opacity, sizeAttenuation: true,
      }));
      pts.position.set(...posArr);
      pts.rotation.z = rotZ;
      scene.add(pts);
      return pts;
    };
    ...
    // ── Nebulose ──────────────────────────────────────────────────────────────
    [
      { color: 0x6633aa, spread: [14, 6, 3], pos: [-5,  3, -12], count: 400, opacity: 0.18 },
      { color: 0x003388, spread: [10, 8, 2], pos: [ 6, -4, -10], count: 350, opacity: 0.15 },
      { color: 0xaa3344, spread: [ 8, 5, 2], pos: [-2, -6,  -8], count: 280, opacity: 0.12 },
    ].forEach(({ color, spread, pos, count, opacity }) => {
      ...
      const np = new THREE.Points(ng, new THREE.PointsMaterial({
        color, size: 0.25, transparent: true, opacity, sizeAttenuation: true,
      }));
      np.position.set(...pos);
      scene.add(np);
    });
    ...
    missions.forEach((m, i) => {
      ...
      const radius    = done ? 0.14 : available ? 0.11 : 0.08;
      ...
      mesh.userData = { mission: m, done, available, locked };
      scene.add(mesh);
      meshes.push(mesh);

      // hit area invisibile più grande
    ...
    const animate = () => {
      ...
      if (++labelTick % 4 === 0) computeLabels();
    };
    animate();
    ...
    const onMouseMove = (e) => {
      ...
      const dx = e.clientX - s.mouseDownX;
      const dy = e.clientY - s.mouseDownY;
      if (Math.abs(dx) > 3 || Math.abs(dy) > 3) s.hasDragged = true;
      const f = s.zoom / window.innerWidth * 1.8;
      s.targetPan.x = s.panAtDown.x - dx * f;
      s.targetPan.y = s.panAtDown.y + dy * f;
      clampPan();
    };
    const onMouseUp = (e) => {
      if (!s.isDragging) return;
    ...
    const onTouchStart = (e) => {
      if (e.touches.length === 1) {
        ...
        s.touchPanAtDown  = { x: s.targetPan.x, y: s.targetPan.y };
        s.touchHasDragged = false;
        s.pinchDist       = null;
    ...
    const onTouchMove = (e) => {
      ...
      if (e.touches.length === 1) {
        ...
        s.targetPan.x = s.touchPanAtDown.x - dx * f;
        s.targetPan.y = s.touchPanAtDown.y + dy * f;
        clampPan();
  ...
  return (
    <div
      ...
      style={{ background: "#020b18", cursor: stateRef.current.isDragging ? "grabbing" : "grab" }}
    >
      <canvas ref={canvasRef} className="absolute inset-0 w-full h-full" style={{ touchAction: "none" }} />

      {/* ── Titolo ── */}
      <div className="absolute top-4 left-1/2 -translate-x-1/2 text-center pointer-events-none select-none">
        <div style={{ fontSize: 8, fontWeight: 900, letterSpacing: "0.4em", color: "rgba(56,189,248,0.4)", textTransform: "uppercase" }}>
          Trama Viva
        </div>
        <div style={{ fontWeight: 900, fontSize: isMob ? 14 : 16, color: "rgba(255,255,255,0.72)", lineHeight: 1.2, marginTop: 2 }}>
      ...
      {!panelOpen && (
        <div className="select-none pointer-events-none" style={{
          ...
          {isMob
            ? "Trascina · Pizzica per zoomare · Tocca una stella"
            : "Trascina per navigare · Scroll per zoomare · Clicca una stella"}
        </div>
      )}

      {/* ── Label stelle ── */}
      {labels.map((lp, i) => (
        <div key={i} className="select-none pointer-events-none" style={{
          position: "absolute", left: lp.x, top: lp.y - (isMob ? 22 : 28),
          transform: "translateX(-50%)", textAlign: "center",
        }}>
          <div style={{ fontSize: isMob ? 12 : 14, lineHeight: 1, filter: lp.locked ? "grayscale(1) opacity(0.2)" : "none" }}>
            {lp.mission.emoji}
          </div>
          <div style={{
            fontSize: isMob ? 7 : 9, fontWeight: 800, letterSpacing: "0.04em", marginTop: 2,
            color: lp.locked ? "rgba(255,255,255,0.1)" : "rgba(255,255,255,0.65)",
            maxWidth: isMob ? 60 : 78, lineHeight: 1.2,
            textShadow: lp.locked ? "none" : "0 1px 8px rgba(0,0,0,0.9)",
          }}>
            {lp.mission.title}
          </div>
        </div>
        ...
        {selected && (
          <div style={{ maxWidth: 520, margin: "0 auto" }}>
            <div style={{ display: "flex", gap: 14, alignItems: "flex-start", marginBottom: 14 }}>
              <div style={{
                width: isMob ? 46 : 54, height: isMob ? 46 : 54, borderRadius: 13,
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: isMob ? 20 : 24, flexShrink: 0,
